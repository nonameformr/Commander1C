		...
		#Иначе
			...
			Для Каждого стр Из FSO.Drives Цикл
				Элементы[ИмяЭлемента].СписокВыбора.Добавить(стр.Path, ?(стр.DriveType = 3, стр.Path 
					+ " (" + стр.ShareName + ")", ""), , ПолучитьКартинкуДиска(стр.DriveType));
...
Процедура ПрочитатьСписокДисковСервер(ИмяЭлемента)
	...
	Если СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Windows_x86
		...
		Для Каждого стр Из FSO.Drives Цикл
			Элементы[ИмяЭлемента].СписокВыбора.Добавить(стр.Path, ?(стр.DriveType = 3, стр.Path 
				+ " (" + стр.ShareName + ")", ""), , ПолучитьКартинкуДиска(стр.DriveType)); //, стр.Path + " ["+стр.VolumeName+"]");
		КонецЦикла;
	Иначе
		Элементы[ИмяЭлемента].СписокВыбора.Добавить("/");
...
Процедура ПоказатьРазмерДискаНаКлиенте(ИмяЭлементаДиска, ИмяЭлементаНадписи)
	...
	Если СистемнаяИнформация.ТипПлатформы = ТипПлатформы.Windows_x86
		...
		#КонецЕсли
	Иначе
		РазмерДискаЛинуксКлиент(ИмяЭлементаДиска);
	КонецЕсли;
...
&НаКлиенте
Процедура РазмерДискаЛинуксКлиент(ИмяЭлементаДиска)
...
&НаСервере
Процедура РазмерДискаЛинуксСервер(ИмяЭлементаДиска)
