...
&НаКлиенте
Перем МассивПлатформа, СистемнаяИнформация, Версия1СБольше838, Версия1СБольше8315;
...
Процедура Копирование(ИмяПути = "")
	...
	Попытка
		...
		Иначе
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Копирование каталогов еще не реализовано';"
				+ "en = 'Copying directories has not yet been implemented'");
			Сообщение.Сообщить();
		КонецЕсли;
	Исключение
		Сообщение = Новый СообщениеПользователю;
...
Процедура СписокФайловЛевоВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если НРег(Элемент.ТекущиеДанные.Расширение) = ".epf" Или НРег(Элемент.ТекущиеДанные.Расширение) = ".erf" Тогда
		ЗавершениеПомещенияФайла = Новый ОписаниеОповещения("ЗавершениеПомещенияФайла", ЭтотОбъект, 
			Новый Структура("Расширение", Элемент.ТекущиеДанные.Расширение));
		НачатьПомещениеФайлаНаСервер(ЗавершениеПомещенияФайла, , , , Объект.ПутьЛево + "/" + Элемент.ТекущиеДанные.Файл);
	ИначеЕсли Элемент.ТекущиеДанные.Каталог Тогда
...
Процедура СписокФайловПравоВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если НРег(Элемент.ТекущиеДанные.Расширение) = ".epf" Или НРег(Элемент.ТекущиеДанные.Расширение) = ".erf" Тогда
		ЗавершениеПомещенияФайла = Новый ОписаниеОповещения("ЗавершениеПомещенияФайла", ЭтотОбъект,
			Новый Структура("Расширение", Элемент.ТекущиеДанные.Расширение));
		НачатьПомещениеФайлаНаСервер(ЗавершениеПомещенияФайла, , , , Объект.ПутьПраво + "/" + Элемент.ТекущиеДанные.Файл);
	ИначеЕсли Элемент.ТекущиеДанные.Каталог Тогда
...
Процедура ПрочитатьСписокФайловНаСервере(ИмяЭлемента, Путь)
	...
	Для Каждого НайденныйФайл Из НайденныеФайлы Цикл
		...
		Иначе
			...
			Иначе
				НоваяСтрока.Файл = НайденныйФайл.Имя;
			КонецЕсли;
		КонецЕсли; 
		НоваяСтрока.Расширение = НайденныйФайл.Расширение;
...
Процедура ВыводДанныхФайлаВСписок(КонтекстПоиска)
	...
	Иначе
		НоваяСтрока.Файл = ТекущийФайл.Имя;
	КонецЕсли;
	НоваяСтрока.Расширение = КонтекстПоиска.ТекущийФайл.Расширение;
...
Процедура ПрочитатьСписокДисковЛокально(ИмяЭлемента)
	...
	Иначе
		...
		НачатьЗапускПриложения(Новый ОписаниеОповещения("ПослеЧтенияФайлаНаКлиентеLinux", ЭтотОбъект, ИмяЭлемента),
...
Процедура ПослеЧтенияФайлаНаКлиентеLinux(КодВозврата, ДопПараметры) Экспорт
	...
	Пока СтрокаТекста <> Неопределено Цикл
		...
		МассивСлов = СтрРазделить(СтрокаТекста, " ", Ложь);
...
СистемнаяИнформация = Новый СистемнаяИнформация;
МассивПлатформа = СтрРазделить(СистемнаяИнформация.ВерсияПриложения, ".");
Версия1СБольше838 = Число(МассивПлатформа[1]) = 3 И Число(МассивПлатформа[2]) >= 8;
Версия1СБольше8315 = Число(МассивПлатформа[1]) = 3 = Число(МассивПлатформа[2]) >= 15;
