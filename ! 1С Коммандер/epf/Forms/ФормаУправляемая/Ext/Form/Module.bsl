...
&НаКлиенте
Перем МассивПлатформа, СистемнаяИнформация, ВерсияБольше838;

// Хранит картинки дисков полученные из макета
&НаКлиентеНаСервере
Перем КартинкиДисков;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НовыйРеквизитЛево = Новый РеквизитФормы("КартинкаЛево", Новый ОписаниеТипов("Картинка"), "Объект.СписокФайловЛево");
	ДобавляемыеРеквизиты.Добавить(НовыйРеквизитЛево);
	НовыйРеквизитПраво = Новый РеквизитФормы("КартинкаПраво", Новый ОписаниеТипов("Картинка"), "Объект.СписокФайловПраво");
	...
	КолонкаКартинки.ПутьКДанным = "Объект.СписокФайловЛево.КартинкаЛево";
	...
	КолонкаКартинки.Заголовок = "";
	КолонкаКартинки.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	КолонкаКартинки.ПутьКДанным = "Объект.СписокФайловПраво.КартинкаПраво";
	КолонкаКартинки.Вид = ВидПоляФормы.ПолеКартинки;
	КолонкаКартинки.Ширина = 2;
...
Процедура РазмерДискаКлиентWindows(ИмяЭлементаДиска, ИмяЭлементаНадписи)
	...
	Если Drive.IsReady Тогда
		Попытка
			...
			Элементы[ИмяЭлементаНадписи].Заголовок = "[" + МеткаДиска + "]  свободно " 
				+ Формат(СвободноеМесто / Объект.ЗнаменательДляГигабайт, "ЧДЦ=2") + " ГБ из " 
				+ Формат(ВсегоМеста / Объект.ЗнаменательДляГигабайт, "ЧДЦ=2") + " ГБ (" + ТипФС + ")";
		Исключение
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = ОписаниеОшибки();
			Сообщение.Сообщить();
...
Объект.СтрокаПоддерживаемыхСтроковыхФайлов = ".txt;.log;.ini;.lst;.v8i;.sh;.json;.md;";
СистемнаяИнформация = Новый СистемнаяИнформация;
